...
public class ThreadDispatcherInvokeAsyncTaskWithResultMethodTest
{
	...
	{
		await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.InvokeAsync(func: (Func<Task<int>>)null!));
	}
	...
	public async Task ThrowsWhenFinished()
	{
		...
		await Assert.ThrowsAsync<InvalidOperationException>
			(() => _sut.InvokeAsync(func: (Func<Task<int>>)(() => throw new Exception("test"))));
	}
	...
	{
		var task = Task.Run(() => _sut.Run());
		Func<Task<int>> func = () => throw new Exception("test");
	...
	public async Task InvokesNestedFuncs()
	{
		...
		var task = Task.Run(() => _sut.Run());
		var childFunc = () => Task.FromResult(1);

		async Task<int> ParentFunc()
		{
			var sum = 0;

			for (var i = 0; i < invokedCound; i++)
			{
				sum += await _sut.InvokeAsync(func: childFunc);
			}
	...
	public async Task ThrowsOperationCanceledWhenCancelledFromToken()
	{
		...
		var task2 = _sut.InvokeAsync(func: (Func<Task<int>>)(() => throw new Exception("test")), tokenSource.Token);
